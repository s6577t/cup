...
var statusThemes = {
  ...
  , notice  : { color: 'cyan', backgroundColor: 'darkGrey', bold :true}
...
var shell = {
  status: function (status, action) {
    return {
      message: function (message) {
        ...
        var message = " " + technicolor((action || status), statusThemes[status]) + "\t" + message;
  ...
  , emptyDirectory: function (dir) {
    ...
    this.status('confirm', 'new directory').message(technicolor(dir, {underline: true}));
  ...
  , createFile: function (file, content) {
    ...
    this.status('confirm', 'file created').message(technicolor(file, {underline: true}));
