...
exports = module.exports = (function () {
  ...
  addCommand('server', 'start a cup server', ['port=1212'], function (port) {
    withValidCupfile(function () {
      ...
      sh.status('ok').message('cup server listening on port %d in %s mode', port, server.settings.env);
    })
  });
  ...
  addCommand('scheme', 'list javascripts for the specified scheme', ['scheme'], function (scheme) {
    withValidCupfile(function (cup) {
      cup.javascriptsFor.call(cup, scheme).forEach(function (javascript) {
        console.info(javascript);
      });
    })
  });
  ...
  addCommand('concatenate', 'concatenates lib and src files in the order specified by the cupfile', function () {
    withValidCupfile(function (cup) {
  ...
  addCommand('uglify', 'uglifies the concatenation', function () {
    withValidCupfile(function (cup) {
      var builder = new Builder(cup);
  ...
  addCommand('build', 'uglify and concatenate into the build directory', function () {
    withValidCupfile(function (cup) {
      var builder = new Builder(cup);
      var result = builder.build();
      sh.status('confirm').message("concatenated [%d bytes]:\t" + technicolor('%s', {underline: true}), result.concatenate.size, result.concatenate.path);
      sh.status('confirm').message("uglified [%d bytes, %s]:\t" + technicolor('%s', {underline: true}), result.uglify.size, result.uglify.percentage, result.uglify.path);
    });
  });
